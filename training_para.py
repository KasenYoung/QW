    ...
    def test(model, data, Q, J_all, W_cost, epoch):
        ...
        for _, mask in data('train_mask', 'val_mask', 'test_mask'):
            if args.Original_ot == 'ot':
                ...
                torch.save(J_all[mask,:]@Q,f'./Q/{args.dataset}_softmax.pt')
            elif args.Original_ot == 'Original':
        ...
        if args.net=='GCN':
            ...
            optimizer = torch.optim.Adam(
                [{'params': model.conv1.parameters(), 'weight_decay': args.weight_decay, 'lr': args.lr},
                 ...
                 #{'params': model.Q, 'weight_decay': args.q_linear_delay, 'lr': args.q_linear_lr}
                 {'params': model.q1, 'weight_decay': args.q_linear_delay, 'lr': args.q_linear_lr},
                 {'params': model.l1.parameters(), 'weight_decay': args.q_linear_delay, 'lr': args.q_linear_lr}
                 ])
        elif args.net == 'GIN':
            optimizer = torch.optim.Adam(
                [{'params': model.conv1.parameters(), 'weight_decay': args.weight_decay, 'lr': args.lr},
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lambda_', type=float, default=1.0, help='')
    parser.add_argument('--A_F', action='store_true', help='')
    parser.add_argument('--mlp_layers', type=int, default=2, help='')
    parser.add_argument('--q_linear_lr', type=float, default=0.002, help='q_linear_lr.')
    ...
    parser.add_argument('--sample_graph',type=bool,choices=[True,False],default=False)
    parser.add_argument('--sample_linegraph',type=bool,choices=[True,False],default=False)
    parser.add_argument('--sample_method',type=str,choices=['kmis','random'],default='kmis')
    parser.add_argument('--kmis_k',type=int,default=0)
    parser.add_argument('--random_p',type=float,default=1.0)
    ...
    #SEEDS = [1941488137, 1941488137, 1941488137,1941488137,1941488137,1941488137,1941488137,1941488137,1941488137,1941488137]
    ...
    data = dataset[0]
    '''
    if args.dataset in ['Computers','Squirrel']: #downsampling
        kpool=KMISPooling(k=1)
        newdata=kpool(data.x,data.edge_index)[0]
        data=Data(x=newdata.x,edge_index=newdata.edge_index,edge_attr=newdata.edge_attr,y=data.y)
        print(f'Downsampled graph edges: {data.edge_index.shape}.')
    '''
    if args.dataset in ['Computers','Squirrel']: #downsampling
        if args.sample_graph: #downsample on the graph
        
            if args.sample_method=='kmis':
                pool=downsampling.KMISPooling(k=args.kmis_k)
                newdata=pool(data.x,data.edge_index)[0]
                data=Data(x=newdata.x,edge_index=newdata.edge_index,edge_attr=newdata.edge_attr,y=data.y)
            elif args.sample_method=='random':
                pool=downsampling.Random_EdgeSelection(p=args.random_p)
                newdata=pool(data.x,data.edge_index,data.edge_attr,data.is_directed())
                data=Data(x=newdata.x,edge_index=newdata.edge_index,edge_attr=newdata.edge_attr,y=data.y)
    ...
    for RP in tqdm(range(args.runs)):
        ...
        results.append([test_acc, best_val_acc,best_gnn_acc])
    ...
    test_acc_mean, val_acc_mean,gnn_acc_mean= np.mean(results, axis=0) * 100
    ...
    print(f'test acc mean = {test_acc_mean:.4f} ± {uncertainty * 100:.4f}  \t val acc mean = {val_acc_mean:.4f} \t test gnn acc mean ={gnn_acc_mean:.2f}')

    with open('./paralogs/'+'logs-1_' + args.net + '_QW_' + args.dataset, 'a+') as f:
        f.write(
            'net:{}, Original_ot:{}, train_rate:{}, val_rate:{}, lr:{}, weight_decay:{}, dropout:{}, prop_lr:{}, prop_wd:{}, dprate:{}, lambda_:{}, q_linear_lr:{}, q_linear_delay:{}, A_F:{},sample_graph:{},sample_linegraph:{},sample_method:{},kmis_k:{},random_p:{}\n'.format(
                ...
                args.lambda_, args.q_linear_lr, args.q_linear_delay, args.A_F,
                args.sample_graph,args.sample_linegraph,args.sample_method,args.kmis_k,args.random_p
                ))
        f.write(f'test acc mean = {test_acc_mean:.2f} ± {uncertainty * 100:.2f}  \t val acc mean = {val_acc_mean:.4f} \t test gnn acc mean ={gnn_acc_mean:.2f}')
        f.write('\n')
        f.write('\n')
