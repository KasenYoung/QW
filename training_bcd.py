...
def RunExp(args, dataset, data, Net, percls_trn, val_lb):
    ...
    def train(model, optimizer, data, W_cost, epoch,m,v,interval,q_lr,q_decay):
        model.train()
        optimizer.zero_grad()
        #print(W_cost.shape)
        ...
        optimizer.step()
        Q=Q.detach()
        step=int(Q.shape[0]/interval)
        for i in range(0,interval-1):
            grad,m,v=bcd(W_cost,J_all[data.train_mask],Q,out,data.y[data.train_mask],m,v,q_lr,epoch)
            Q[i*step:(i+1)*step,:]=Q[i*step:(i+1)*step,:]*(1-q_decay)+grad[i*step:(i+1)*step,:]
        grad,m,v=bcd(W_cost,J_all[data.train_mask],Q,out,data.y[data.train_mask],m,v,q_lr,epoch)
        Q[(interval-1)*step:,:]=Q[(interval-1)*step:,:]*(1-q_decay)+grad[(interval-1)*step:,:]
        #del out
        return Q
    
    def bcd(W,J,Q,y_hat,Y,m,v,lr,epoch):
        #print(J.shape,y_hat.shape,Y.shape)
        W_hat=torch.repeat_interleave(W,Q.shape[1],dim=1)
        W_hat[Q<0]*=-1
        
        g_t=torch.matmul(J.T,y_hat-F.one_hot(Y,num_classes=Q.shape[1]))+W_hat
        m=0.9*m+0.1*g_t
        v=0.999*v+0.001*g_t*g_t
        m_hat=m/(1-0.9**(epoch+1))
        v_hat=v/(1-0.999**(epoch+1))
        grad=-lr*m_hat/(v_hat+1e-8)
    ...
    def test(model, data, Q, J_all, W_cost, epoch):
        ...
        for _, mask in data('train_mask', 'val_mask', 'test_mask'):
            if args.Original_ot == 'ot':
                ...
                torch.save(J_all[mask,:]@Q,f'./Q_Bcd/{args.dataset}_softmax.pt')
            elif args.Original_ot == 'Original':
    ...
    m=torch.zeros((int(dataset.data.edge_index.size(1) / 2), dataset.num_classes))
    v=torch.zeros((int(dataset.data.edge_index.size(1) / 2), dataset.num_classes))
    m=m.to(args.device)
    v=v.to(args.device)
    ...
    for epoch in range(args.epochs):
        t_st = time.time()
        Q = train(model, optimizer, data, W_cost, epoch,m,v,args.interval, args.q_linear_lr,args.q_linear_delay)
        torch.save(Q,f'./Q_Bcd/{args.dataset}_Q.pt')
...
if __name__ == '__main__':
    ...
    with open('./Bcd_logs/'+'logs-1_' + args.net + '_QW_' + args.dataset+'original', 'a+') as f:
        f.write(
