...
class GAT_Net(torch.nn.Module):
    def __init__(self, dataset, args):
        ...
        self.q1 = Parameter(torch.Tensor(int(dataset.data.edge_index.size(1) / 2), 1))
        self.l1=Linear(1,dataset.num_classes)
        self.reset_parameters()
    ...
    def reset_parameters(self):
        ...
        #self.gcn2_dual.reset_parameters()
        nn.init.kaiming_uniform_(self.q1)

    def forward(self, data, epoch):
        #print(self.dual_dataset.edge_index)
        
        #print(self.Q1.shape)
        self.Q=F.relu(self.l1(self.q1))
        x, edge_index = data.x, data.edge_index
        x = F.dropout(x, p=self.dropout, training=self.training)
