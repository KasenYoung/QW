...
import downsampling
from linegraph import LineGraph2,LineGraph3
...
def RunExp(args, dataset, data,dual_data, Net, percls_trn, val_lb):
    
    def train(model, optimizer, data,dual_data, W_cost, epoch):
        
        print(sum(p.numel() for p in model.parameters() if p.requires_grad))
        model.train()
        optimizer.zero_grad()
        out_model = model(data, dual_data,epoch)
        out, Q = out_model[0][data.train_mask], out_model[1]
        #print(out.shape,Q.shape)
        if args.Original_ot == 'ot':
            #print((out + J_all[data.train_mask] @ Q).shape,data.y[data.train_mask].shape)
    ...
    def test(model, data,dual_data, Q, J_all, W_cost, epoch):
        model.eval()
        logits, accs, losses, preds = model(data, dual_data,epoch)[0], [], [], []
    ...
    for epoch in range(args.epochs):
        t_st = time.time()
        Q = train(model, optimizer, data,dual_data, W_cost, epoch)
        time_epoch = time.time() - t_st  # each epoch train times
        time_run.append(time_epoch)
        [train_acc, val_acc, tmp_test_acc], preds, [train_loss, val_loss, tmp_test_loss] = test(model, data,dual_data, Q, J_all, W_cost, epoch)
...
if __name__ == '__main__':
    ...
    for file in os.listdir('./subgraphs/') :
        if args.dataset.lower() in file:
            pt.append('./subgraphs/'+file)
    ...
    for fp in pt:
        ...
        line_graph_trans = LineGraph3()
        dual_data = line_graph_trans(copy.copy(data))
        #print(dual_data.x.shape,dual_data.edge_index.shape)
        dual_data = dual_data.detach()
        
        dual_data=dual_data.to(torch.device('cuda:' + str(args.device) if torch.cuda.is_available() else 'cpu'))
        data=data.to(device)
        ...
        for RP in tqdm(range(args.runs)):
            ...
            test_acc, best_val_acc, theta_0, time_run = RunExp(args, dataset, data, dual_data,Net, percls_trn, val_lb)
        ...
        total_test.append(test_acc_mean)
        total_uncertainty.append(uncertainty)
        total_val.append(val_acc_mean)
        '''
        with open('./largelogs/'+'logs-1_' + args.net + '_QW_' + args.dataset, 'a+') as f:
            f.write('Dataset:{},subgraph:{}.'.format(args.dataset,file))
            f.write(
                'net:{}, Original_ot:{}, train_rate:{}, val_rate:{}, lr:{}, weight_decay:{}, dropout:{}, prop_lr:{}, prop_wd:{}, dprate:{}, lambda_:{}, q_linear_lr:{}, q_linear_delay:{}, A_F:{},\n'.format(
    ...
    with open('./largelogs/'+'logs-2_' + args.net + '_QW_' + args.dataset, 'a+') as f:
            #f.write('Dataset:{},subgraph:{}.'.format(args.dataset,file))
            f.write(
                'net:{}, Original_ot:{}, train_rate:{}, val_rate:{}, lr:{}, weight_decay:{}, dropout:{}, prop_lr:{}, prop_wd:{}, dprate:{}, lambda_:{}, q_linear_lr:{}, q_linear_delay:{}, A_F:{},\n'.format(
                    args.net, args.Original_ot, args.train_rate, args.val_rate,
            ...
            f.write(f'test acc mean = {total_test.mean():.2f} ± {total_uncertainty.mean()*100:.2f}  \t val acc mean = {total_val.mean():.4f}')
            f.write('\n')
            f.write('\n')
